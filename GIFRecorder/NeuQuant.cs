...
namespace GIFRecorder
{
    public class NeuQuant
    {
        ...
        protected int initrad;// = (netsize >> 3); /* for 256 cols, radius starts */
        ...
        protected int initradius;// = (initrad * radiusbias); /* and decreases by a */
        ...
        protected byte[] m_byPixels; /* the input image itself */
        //protected int m_nPixelCount; /* lengthcount = H*W*3 */
        ...
        /* Initialise network in range (0,0,0) to (255,255,255) and set parameters
           ----------------------------------------------------------------------- */
        //public NeuQuant(byte[] thepic, int len, int sample,int nColorCount) {
        ...
        public byte[] GetColorMap(byte[] thepic, int sample, int nColorCount) {
            m_byPixels = thepic;
            samplefac = sample;
            netsize = nColorCount;
            maxnetpos = (netsize - 1);
        ...
        public void Learn() {
            ...
            if (m_byPixels.Length < minpicturebytes)
                samplefac = 1;
            alphadec = 30 + ((samplefac - 1) / 3);
            p = m_byPixels;
            pix = 0;
            lim = m_byPixels.Length;
            samplepixels = m_byPixels.Length / (3 * samplefac);
            ...
            if (m_byPixels.Length < minpicturebytes)
                step = 3;
            else if ((m_byPixels.Length % prime1) != 0)
                step = 3 * prime1;
            else {
                if ((m_byPixels.Length % prime2) != 0)
                    step = 3 * prime2;
                else {
                    if ((m_byPixels.Length % prime3) != 0)
                        step = 3 * prime3;
                    else
                        step = 3 * prime4;
                }
